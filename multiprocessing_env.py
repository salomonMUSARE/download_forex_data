...
class Environment(Process):
    ...
    def run(self):
        ...
        elyse = 0
        while True:
            reset, net_worth, episode_orders , ggoods, bbads = 0, 0, 0, 0, 0
            ...
            if done or self.env.current_step == self.env.end_step:
                ...
                ggoods  = self.env.ggoods
                bbads = self.env.bbads  
                episode_orders = self.env.episode_orders
                
                # if elyse == 2:
                #     elyse = 0
                self.env.reset(env_steps_size = self.training_batch_size)
                # else:
                #     self.env.reset_few()
                reset = 1

            self.child_conn.send([state, reward, done, reset, net_worth, episode_orders, ggoods, bbads])
...
def train_multiprocessing(CustomEnv, agent, train_df, train_df_nomalized, num_worker=4, training_batch_size=500, visualize=False, EPISODES=10000):
    ...
    episode = 0
    toto_orders = deque(maxlen=100)
    good_bad = deque(maxlen=100)
    toto_ggoods = deque(maxlen=100)
    total_average = deque(maxlen=100) # save recent 1000 episodes net worth

    best_average = 1.1 # used to track best average net worth

    for idx in range(num_worker):
        parent_conn, child_conn = Pipe()
        env = CustomEnv(train_df, train_df_nomalized, lookback_window_size=agent.lookback_window_size, Render_range=60)
    ...
    while episode < EPISODES:
        ...
        for worker_id, parent_conn in enumerate(parent_conns):
            next_state, reward, done, reset, net_worth, episode_orders, ggoods, bbads = parent_conn.recv()
            ...
            if reset:
                ...
                average = sum(total_average)
                performance = 0
                toto_ggoods.append(ggoods)
                toto_orders.append(episode_orders)
                toto_ggoodssum = sum(toto_ggoods)
                toto_orderssum = sum(toto_orders)
                if bbads != 0:
                    good_bad.append((ggoods/bbads))
                good_bad_average = np.average(good_bad)

                
                # if not ggoods + bbads == episode_orders:
                #     print("coorect bro")
                if toto_orderssum != 0:
                    performance = (toto_ggoodssum/toto_orderssum)*100

                agent.writer.add_scalar('Data/average net_worth', average, episode)
                agent.writer.add_scalar('Data/episode_orders', episode_orders, episode)
                
                print("episode: {:<5} worker: {:<2} reward: {:<7.2f} average: {:<7.2f} orders: {} performance: {:<7.2f}(%)    good/bad: {:<7.2f}".format(episode, worker_id, net_worth, average, episode_orders,performance,good_bad_average ))
                if best_average < average: 
                    best_average = average + 2000
                    print("Saving model")
